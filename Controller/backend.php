...
class Backend
{
    ...
    function loginUser($tableName, $column1, $value1, $column2 , $value2)
    {
        ...
        foreach ($allData as $row) {
            if ($row[$column1] == $value1 && $this->decryptPassword($value2, $row[$column2])) {
				...
				if(!$this->addTokenRow([$row["token"],true,$row["id"]])){
					$this->returnExecQueryMessage('Unable to insert new user token in token table !!!', false);
				}
	...
	//function to add token to token table
	function addTokenRow($valuesArray){
		if($this->db->insert("insert into token (token,status,userId) values ('$valuesArray[0]','$valuesArray[1]','$valuesArray[2]')")){
			return true;
		}else{ return false; }
	}
